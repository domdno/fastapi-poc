...
class ServiceSubscriptionBasic(BaseModel):
    """
    Patient service offered by BI directly or through its partners to which a patient can sign up to.  
    """ # noqa: E501
    ...
    __properties: ClassVar[List[str]] = ["serviceType", "subscriptionStatus", "subscriptionDate"]

    @field_validator('service_type')
    def service_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('demoDeviceAutoInjection', 'demoDevicePrefilledSyringe',):
            raise ValueError("must be one of enum values ('demoDeviceAutoInjection', 'demoDevicePrefilledSyringe')")
        return value

    @field_validator('subscription_status')
    def subscription_status_validate_enum(cls, value):
        """Validates the enum"""
        if value not in ('optIn', 'optOut', 'oneTimeServiceComplete',):
            raise ValueError("must be one of enum values ('optIn', 'optOut', 'oneTimeServiceComplete')")
        return value
    ...
    def from_json(cls, json_str: str) -> Self:
        """Create an instance of ServiceSubscriptionBasic from a JSON string"""
    ...
    def from_dict(cls, obj: Dict) -> Self:
        """Create an instance of ServiceSubscriptionBasic from a dict"""
        if obj is None:
            return None
